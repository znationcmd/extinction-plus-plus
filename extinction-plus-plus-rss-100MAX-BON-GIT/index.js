...
const pgSync = require('./shared/pg-sync');
...
client.once('clientReady', () => {
  ...
  pgSync.ensureTables().then(ok => { if (ok) console.log('✅ PostgreSQL synchronisé'); }).catch(e => console.error('PostgreSQL sync error:', e.message));
});
  ...
  try {
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId.startsWith('wlmodal|')) {
        ...
        const whitelistRequest = { id:`${Date.now()}_${interaction.user.id}`, guildId:interaction.guildId, userId:interaction.user.id, server, map, plateforme, pseudo, status:'pending', createdAt:new Date().toISOString() };
        db.pendingWhitelist.push(whitelistRequest);
        saveDb(db);
        await pgSync.addWhitelistRequest(whitelistRequest).catch(e => console.error('PostgreSQL whitelist error:', e.message));
...
Le token est chiffré dans la base.`);
      ...
      if (sub === 'link-server') {
        ...
        saveDb(db);
        await pgSync.upsertServer(server).catch(e => console.error('PostgreSQL nitrado server error:', e.message));
...
${answer}`);
    ...
    if (interaction.commandName === 'serveur') {
      ...
      if (sub === 'ajouter') {
        ...
        const serverRecord = { id:`${Date.now()}_${Math.random().toString(36).slice(2,6)}`, guildId: interaction.guildId, game, name, map, platform, provider, nitradoId, nitradoServiceId:nitradoId, ip, port, rconHost, rconPort, rconPassword, image, enabled:true, createdAt:new Date().toISOString() };
        cfg.servers.push(serverRecord);
        saveDb(db);
        await pgSync.upsertServer(serverRecord).catch(e => console.error('PostgreSQL server error:', e.message));

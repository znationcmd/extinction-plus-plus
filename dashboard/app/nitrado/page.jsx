...
export default function Page() {
  const db = readDb();
  const items = Array.isArray(db.nitradoServers) ? db.nitradoServers : [];

  return (
    <Shell>
      <h2 className="mb-6 text-4xl font-black">Nitrado Sync</h2>

      <div className="card mb-6">
        <p className="text-white/70">Connexion Nitrado, détection serveur/map et synchronisation.</p>
        ...
        {items.map((item) => (
          <div key={item.id || item.name || JSON.stringify(item)} className="card">
            ...
            <div className="mt-3 space-y-1 text-white/70">
              <p>Serveur : {item.name || '—'}</p>
<p>Service ID : {item.serviceId || '—'}</p>
<p>Map : {item.map || '—'}</p>
<p>Plateforme : {item.platform || '—'}</p>
